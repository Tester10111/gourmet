...
// --- Helper Icons ---
const LoyaltyPointsIcon = () => <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-yellow-400"><path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5"></path></svg>;
...
const premiumStyles = `
  ...
  * { font-family: 'Poppins', sans-serif; }
  ...
  @keyframes glow { 0%, 100% { filter: drop-shadow(0 0 6px rgba(251, 191, 36, 0.7)); } 50% { filter: drop-shadow(0 0 16px rgba(251, 191, 36, 1)); } }
  ...
  .premium-shadow { box-shadow: 0 20px 60px rgba(0,0,0,0.5), 0 0 80px rgba(59, 130, 246, 0.2); }
  .glass-effect { background: rgba(30, 41, 59, 0.6); backdrop-filter: blur(20px); border: 1px solid rgba(255, 255, 255, 0.15); }
...
const QuickBetButtons = ({ wager, setWager, balance, disabled }) => (
    <div className="flex gap-2 mt-3">
        <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => setWager(Math.max(1, Math.floor(wager / 2)))} disabled={disabled} 
            className="flex-1 text-sm bg-gradient-to-r from-slate-700 to-slate-600 hover:from-slate-600 hover:to-slate-500 px-4 py-3 rounded-xl font-black disabled:opacity-30 transition-all text-white shadow-lg disabled:hover:scale-100">
            ½ HALF
        </motion.button>
        ...
        <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => setWager(Math.floor(balance))} disabled={disabled} 
            className="flex-1 text-sm bg-gradient-to-r from-yellow-600 to-yellow-500 hover:from-yellow-500 hover:to-yellow-400 px-4 py-3 rounded-xl font-black disabled:opacity-30 transition-all text-slate-900 shadow-lg disabled:hover:scale-100">
            💰 MAX
        </motion.button>
    </div>
);

// --- Game: Fruit Frenzy (REFACTORED & FIXED) ---
const FruitFrenzy = ({ balance, setBalance, playSound, logGameResult }) => {
    const REELS_COUNT = 5;
    const ROWS_COUNT = 3;
    ...
    const VIRTUAL_REELS = useMemo(() => {
        const createReel = (weights) => Object.entries(weights).flatMap(([symbol, count]) => Array(count).fill(symbol));
        return [
            createReel({ '🍒': 35, '🍋': 30, '🍊': 25, '🍇': 20, '💎': 10, '7️⃣': 1, '✨': 2 }),
            createReel({ '🍒': 30, '🍋': 25, '🍊': 20, '🍇': 18, '💎': 8, '7️⃣': 2, '✨': 2 }),
            createReel({ '🍒': 25, '🍋': 20, '🍊': 18, '🍇': 15, '💎': 5, '7️⃣': 3, '✨': 1 }),
            createReel({ '🍒': 20, '🍋': 18, '🍊': 15, '🍇': 12, '💎': 4, '7️⃣': 4, '✨': 3 }),
            createReel({ '🍒': 20, '🍋': 18, '🍊': 15, '🍇': 12, '💎': 4, '7️⃣': 5, '✨': 4 }),
        ];
    }, []);

    // Initialize reels with random symbols on mount
    useEffect(() => {
        setVisualReelStrips(VIRTUAL_REELS.map(reel => {
            const strip = [];
            for (let i=0; i < 50; i++) { // Create a long strip for smooth animation
    ...
    const spin = () => {
        ...
        // 1. Determine the final outcome logically BEFORE animating
        for (let i = 0; i < REELS_COUNT; i++) {
            ...
            // 2. Prepare visual strips for animation
            // The strip contains random symbols, ending with the correct final symbols.
            const animationStrip = [];
            const animationLength = 50 + i * 5; // variable length for visual effect
            for (let k = 0; k < animationLength; k++) {
                animationStrip.push(reel[Math.floor(Math.random() * reel.length)]);
            }
            animationStrip.push(...finalReelView); // Add final symbols to the end
            newVisualReels.push(animationStrip);
        }

        setVisualReelStrips(newVisualReels);

        // 3. Animate each reel to its final position
        ...
        // 4. Calculate winnings after the animation finishes
        setTimeout(() => {
            const paylines = [
                finalReels.map(r => r[0]), // Top
                finalReels.map(r => r[1]), // Middle
                finalReels.map(r => r[2]), // Bottom
            ];
            ...
            paylines.forEach((line, lineIndex) => {
                ...
                if (!PAYTABLE[lineSymbol]) return; // Skip non-paying symbols like SCATTER

                let matchCount = 1;
                for (let i = 1; i < line.length; i++) {
                    if (line[i] === lineSymbol) matchCount++;
                    else break;
                }

                if (PAYTABLE[lineSymbol][matchCount]) {
                    totalWinnings += wager * PAYTABLE[lineSymbol][matchCount];
    ...
    return (
        <div className="flex flex-col items-center p-8 bg-gradient-to-br from-purple-900/90 via-slate-900 to-slate-900 rounded-3xl shadow-2xl w-full max-w-2xl mx-auto border-2 border-purple-500/30 premium-shadow animate-scale-in">
            ...
            {showPaytable && (
                 <div className="absolute inset-0 bg-black/80 z-30 flex items-center justify-center p-4 animate-fade-in" onClick={() => setShowPaytable(false)}>
                    <div className="glass-effect rounded-2xl p-6 border-2 border-yellow-400/50 w-full max-w-md">
                        <h2 className="text-3xl font-black text-center mb-4 text-yellow-300">Paytable</h2>
                        <div className="space-y-2">
                            {Object.entries(PAYTABLE).map(([symbol, payouts]) => (
                                <div key={symbol} className="flex items-center justify-between bg-slate-900/50 p-2 rounded-lg">
                                    <div className="text-4xl">{symbol}</div>
                                    <div className="flex gap-4 text-right">
                                        <div><span className="font-bold text-lg">5x</span> <span className="text-yellow-400">{payouts[5]}x</span></div>
                                        <div><span className="font-bold text-lg">4x</span> <span className="text-yellow-400">{payouts[4]}x</span></div>
                                        <div><span className="font-bold text-lg">3x</span> <span className="text-yellow-400">{payouts[3]}x</span></div>
                                    </div>
                                </div>
                            ))}
                        </div>
            ...
            <div className="w-full bg-gradient-to-b from-purple-700 via-purple-800 to-purple-900 rounded-3xl p-6 mb-6 border-4 border-yellow-500/50 shadow-2xl relative overflow-hidden">
                <div className="shimmer absolute inset-0"></div>
                <div className="bg-slate-900 rounded-2xl p-5 flex justify-center gap-3 overflow-hidden h-[260px] relative">
                     {winningLines.includes(0) && <div className="absolute top-[40px] left-0 w-full h-1 bg-yellow-400 z-20 animate-pulse-glow"></div>}
                     {winningLines.includes(1) && <div className="absolute top-1/2 left-0 w-full h-1 bg-yellow-400 -translate-y-1/2 z-20 animate-pulse-glow"></div>}
                     {winningLines.includes(2) && <div className="absolute bottom-[40px] left-0 w-full h-1 bg-yellow-400 z-20 animate-pulse-glow"></div>}
                    {visualReelStrips.map((strip, i) => (
                        <div key={i} className="w-1/5 h-full overflow-hidden">
                            <div ref={el => reelRefs.current[i] = el} style={{ transform: 'translateY(0px)' }} className="relative">
                                {strip.map((symbol, j) => (
                                    <div key={j} className="w-full h-[80px] flex items-center justify-center text-5xl transition-all">{symbol}</div>
                                ))}
                            </div>
                        </div>
                    ))}
                 ...
                 {showResult && lastWin > 0 && (
                    <div className="absolute inset-0 bg-black/70 flex items-center justify-center z-20 backdrop-blur-sm rounded-3xl">
                        <div className="text-center animate-bounce-in">
                            <div className="text-7xl font-black text-yellow-400 animate-neon mb-2">WIN!</div>
                            <div className="text-5xl font-black text-green-300 animate-pulse-glow">+${lastWin.toFixed(2)}</div>
                            <div className="text-xl text-yellow-200 mt-2">{(lastWin/wager).toFixed(2)}x</div>
                        </div>
                    </div>
                )}
            </div>
             <div className="w-full">
                <div className="flex justify-between items-center mb-2">
                    ...
                    <button onClick={() => setShowPaytable(true)} className="text-sm text-yellow-400 font-bold hover:underline">Paytable</button>
                </div>
                <input type="number" value={wager} onChange={e => setWager(Math.max(0, Math.min(balance, parseInt(e.target.value) || 0)))} disabled={spinning} className="w-full bg-slate-800 p-5 rounded-2xl border-2 border-purple-500/40 text-2xl font-black shadow-inner mb-3 focus:border-purple-400 focus:outline-none transition-all"/>
                <QuickBetButtons wager={wager} setWager={setWager} balance={balance} disabled={spinning} />
                <button onClick={spin} disabled={spinning || wager <= 0 || wager > balance} className="w-full mt-4 bg-gradient-to-r from-yellow-500 via-yellow-400 to-yellow-500 text-slate-900 p-6 rounded-2xl text-3xl font-black shadow-2xl hover:scale-105 hover:shadow-yellow-500/50 disabled:opacity-50 disabled:hover:scale-100 transition-all animate-glow">
...
// --- Game: Sugar Scratch (High Volatility, 85% RTP) ---
const SugarScratch = ({ balance, setBalance, playSound, logGameResult }) => {
    ...
    const PRIZE_TIERS = useMemo(() => [
        { prize: 250000, prob: 0.000002 },
        { prize: 25000, prob: 0.00001 },
        { prize: 1000, prob: 0.00025 },
        { prize: 100, prob: 0.005 },
        { prize: 20, prob: 0.05 },
        { prize: 10, prob: 0.125 },
        { prize: 5, prob: 0.1 },
    ], []);
    ...
    const startGame = () => {
        if (balance < TICKET_PRICE || playing) return;
        playSound('playButtonClick');
        ...
        for (const tier of PRIZE_TIERS) {
            ...
            if (rand < cumulativeProb) { finalTier = tier; break; }
        }

        const allPossibleNumbers = Array.from({ length: 50 }, (_, i) => i + 1);
        const shuffled = allPossibleNumbers.sort(() => 0.5 - Math.random());
        
        const winners = shuffled.slice(0, 4).map(n => ({ number: n, prize: 0 }));
        let playerNumPool = shuffled.slice(4);
        
        if (finalTier) {
            winners[0].prize = finalTier.prize;
            const winnerIndexInPlayerPool = Math.floor(Math.random() * 12);
            playerNumPool[winnerIndexInPlayerPool] = winners[0].number;
        }

        const winningNumsOnly = winners.map(wn => wn.number);
        playerNumPool.slice(0, 12).forEach((num, index) => {
            if (!winningNumsOnly.includes(num)) {
                if (Math.random() < 0.4) { 
                    const randomWinner = winningNumsOnly[Math.floor(Math.random() * winningNumsOnly.length)];
                    let nearMissNum = randomWinner + (Math.random() < 0.5 ? 1 : -1);
                    if (nearMissNum < 1) nearMissNum = 1;
                    if (nearMissNum > 50) nearMissNum = 50;
                    if (!playerNumPool.includes(nearMissNum) && !winningNumsOnly.includes(nearMissNum)) {
                        playerNumPool[index] = nearMissNum;
                    }
                }
            }
        });
        ...
        setPlayerNumbers(playerNumPool.slice(0, 12).sort(() => 0.5 - Math.random()).map(n => ({
            number: n,
            isWinner: winningNumsOnly.includes(n) && winners.find(w => w.number === n)?.prize > 0
        })));
    };
    ...
    const revealResults = useCallback(() => {
        let finalWinnings = 0;
        playerNumbers.forEach(pn => {
            const match = winningNumbers.find(wn => wn.number === pn.number && wn.prize > 0);
            if (match) {
                finalWinnings += match.prize;
            }
        });
        ...
        logGameResult('Sugar Scratch', TICKET_PRICE, finalWinnings);
        setTimeout(() => setPlaying(false), 3000);
    }, [playerNumbers, winningNumbers, setBalance, playSound, logGameResult]);

    return (
        <div className="flex flex-col items-center p-8 bg-gradient-to-br from-pink-900/90 via-slate-900 to-slate-900 rounded-3xl shadow-2xl w-full max-w-2xl mx-auto border-2 border-pink-500/30 premium-shadow animate-scale-in">
            <Confetti show={showConfetti} />
            <div className="w-full glass-effect p-4 rounded-2xl mb-4 text-center">
                <h3 className="text-xl font-bold text-yellow-300 mb-2">Winning Numbers</h3>
                <div className="flex justify-center gap-4">
                    {Array.from({length: 4}).map((_, i) => (
                        <div key={i} className="flex flex-col items-center">
                           <div className={`w-16 h-16 rounded-full flex items-center justify-center text-2xl font-black transition-all duration-500 ${revealed && winningNumbers[i]?.prize > 0 ? 'bg-green-500 text-white animate-pulse-glow' : 'bg-slate-900/50 text-yellow-400'}`}>
                                {playing ? winningNumbers[i]?.number : '?'}
                            </div>
                           {revealed && winningNumbers[i]?.prize > 0 && <div className="text-xs text-green-300 font-bold mt-1">${winningNumbers[i]?.prize}</div>}
                        </div>
                    ))}
                </div>
            </div>
            ...
            <div className="w-full grid grid-cols-4 gap-4 mb-6">
                {Array.from({ length: 12 }).map((_, i) => (
                    <div key={i} onClick={() => scratch(i)}
                         className="aspect-square rounded-2xl bg-gray-400 cursor-pointer flex items-center justify-center text-3xl font-black relative overflow-hidden shadow-lg hover:scale-105 transition-all">
                        ...
                        <span className={`transition-all duration-300 ${scratched.has(i) ? 'scale-100' : 'scale-0'} ${playerNumbers[i]?.isWinner && revealed ? 'text-green-300 animate-neon' : ''}`}>
                            {playing ? playerNumbers[i]?.number : ''}
                        </span>
                    </div>
                ))}
            </div>
            ...
            <div className="w-full">
                ...
                {playing && !revealed && <button onClick={revealAll} className="w-full bg-yellow-500 text-slate-900 p-3 rounded-xl mb-3 font-bold">Reveal All</button>}
                <button onClick={playing ? null : startGame} disabled={playing || balance < TICKET_PRICE} 
                    className="w-full bg-gradient-to-r from-pink-500 via-purple-500 to-pink-500 text-white p-6 rounded-2xl text-3xl font-black shadow-2xl hover:scale-105 hover:shadow-pink-500/50 disabled:opacity-50 disabled:hover:scale-100 transition-all">
                    {playing ? 'Scratching...' : `Buy New Card ($${TICKET_PRICE.toFixed(2)})`}
                </button>
            </div>
        </div>
    );
...
// --- Game: Rocket Kebab (High Volatility, 85% RTP) ---
const RocketKebab = ({ balance, setBalance, playSound, logGameResult }) => {
    const [wager, setWager] = useState(10);
    ...
    useEffect(() => {
        ...
        if (gameState === 'running') {
            ...
            const r = Math.random();
            let crashPointValue = (1 - 0.85) / (1 - r * 0.85);
    ...
    return (
        <div className="flex flex-col items-center p-8 bg-gradient-to-br from-orange-900/90 via-slate-900 to-slate-900 rounded-3xl shadow-2xl w-full max-w-2xl mx-auto border-2 border-orange-500/30 premium-shadow animate-scale-in">
            ...
            <div className="relative w-full h-96 bg-gradient-to-b from-indigo-950 to-purple-950 rounded-3xl mb-6 flex items-center justify-center overflow-hidden border-4 border-orange-500/30 shadow-2xl">
                ...
                <div className="text-center z-10 relative">
                    {gameState === 'crashed' || (gameState === 'waiting' && cashoutMultiplier === 0) ? (
                        <div className="animate-shake">
                            <div className="text-7xl font-black text-red-500 mb-3 animate-neon">CRASHED!</div>
                            <div className="text-4xl font-bold text-red-400">@{crashPoint.toFixed(2)}x</div>
                        </div>
                    ) : gameState === 'cashedOut' || (gameState === 'waiting' && cashoutMultiplier > 0) ? (
                        <div className="animate-bounce-in">
                            <div className="text-7xl font-black text-green-400 mb-3 animate-neon">SUCCESS!</div>
                            <div className="text-4xl font-bold text-green-300">@{cashoutMultiplier.toFixed(2)}x</div>
                            <div className="text-3xl font-black text-yellow-300 mt-2">+${(placedBet * cashoutMultiplier).toFixed(2)}</div>
                        </div>
                    ) : (
                        <div className={`text-8xl font-black tabular-nums transition-all duration-200 ${multiplier > 10 ? 'text-yellow-400 animate-neon' : multiplier > 5 ? 'text-orange-400' : 'text-white'}`}>
            ...
            <div className="w-full">
                {gameState === 'running' && placedBet ? (
                    <button onClick={cashOut} className="w-full bg-gradient-to-r from-yellow-500 via-yellow-400 to-yellow-500 text-slate-900 p-6 rounded-2xl text-3xl font-black shadow-2xl hover:scale-105 transition-all animate-glow">
                        EJECT @ {(placedBet * multiplier).toFixed(2)}
                    </button>
                ) : (
                    <>
                        <label className="text-sm text-slate-300 mb-2 block font-bold">Bet Amount</label>
                        <input type="number" value={wager} onChange={e => setWager(Math.max(0, Math.min(balance, parseInt(e.target.value) || 0)))} disabled={gameState !== 'betting'} className="w-full bg-slate-800 p-5 rounded-2xl border-2 border-orange-500/40 text-2xl font-black shadow-inner mb-3 focus:border-orange-400 focus:outline-none transition-all"/>
                        <QuickBetButtons wager={wager} setWager={setWager} balance={balance} disabled={gameState !== 'betting'} />
                        <button onClick={placeBet} disabled={gameState !== 'betting' || placedBet} className="w-full mt-4 bg-gradient-to-r from-green-500 via-emerald-500 to-green-500 text-white p-6 rounded-2xl text-3xl font-black shadow-2xl hover:scale-105 hover:shadow-green-500/50 disabled:opacity-50 disabled:hover:scale-100 transition-all">
...
// --- Game: Candy Drop (High Volatility, 85% RTP) --- [REWORKED]
const CandyDrop = ({ balance, setBalance, playSound, logGameResult }) => {
    const [wager, setWager] = useState(10);
    ...
    const animationSpeed = 150; // ms per row
    ...
    return (
        <div className="flex flex-col items-center p-8 bg-gradient-to-br from-blue-900/90 via-slate-900 to-slate-900 rounded-3xl shadow-2xl w-full max-w-2xl mx-auto border-2 border-blue-500/30 premium-shadow animate-scale-in">
            <Confetti show={showConfetti} />
            <div className="w-full flex justify-center gap-3 mb-5 glass-effect p-3 rounded-2xl border-2 border-blue-400/20">
                {['low', 'medium', 'high'].map(r => (
                    <motion.button 
                        ...
                        whileTap={{ scale: 0.95 }}
                        className={`px-8 py-3 rounded-xl text-base font-black capitalize transition-all ${
                            risk === r 
            ...
            <div className="relative w-full aspect-square bg-gradient-to-b from-indigo-950 to-purple-950 rounded-3xl mb-6 flex flex-col justify-between p-4 border-4 border-blue-500/30 shadow-2xl overflow-hidden">
                <AnimatePresence>
                    {batchTotalWin !== null && !isDropping && (
                        ...
                        >
                            <div className="text-center">
                                <div className={`text-6xl font-black mb-3 animate-neon ${
                                    batchTotalWin >= (wager * ballCount) ? 'text-green-400' : 'text-yellow-400'
                                }`}>
                                    Total Win!
                                </div>
                ...
                <div className="flex justify-center relative z-10">
                    {multipliers.map((m, i) => {
                        ...
                        return (
                            <div key={i} 
                                className={`text-xs font-black text-center w-[calc(100%/14)] py-2 rounded-xl transition-all duration-300 ${
                                    isHit && !isDropping
                                        ? 'scale-125 bg-yellow-400/70 shadow-lg shadow-yellow-400/50 animate-pulse-glow' 
                                        : 'bg-slate-800/70'
                                }`} 
                                style={{ 
                                    color: m >= 5 ? '#4ade80' : m > 1 ? '#fbbf24' : m < 1 ? '#f87171' : '#cbd5e1',
                                    border: `2px solid ${m >= 5 ? '#4ade80' : m > 1 ? '#fbbf24' : m < 1 ? '#f87171' : '#475569'}`
                                }}>
                                {m}x
                            </div>
                        );
            ...
            <div className="w-full">
                <div className="grid grid-cols-2 gap-3">
                    <div>
                        <label className="text-sm text-slate-300 mb-2 block font-bold">Bet Amount</label>
                        <input type="number" value={wager} onChange={e => setWager(Math.max(0, Math.min(balance, parseInt(e.target.value) || 0)))} disabled={isDropping} 
                            className="w-full bg-slate-800 p-5 rounded-2xl border-2 border-blue-500/40 text-2xl font-black shadow-inner focus:border-blue-400 focus:outline-none transition-all"/>
                    </div>
                    <div>
                        <label className="text-sm text-slate-300 mb-2 block font-bold">Candies</label>
                        <select value={ballCount} onChange={e => setBallCount(Number(e.target.value))} disabled={isDropping}
                            className="w-full bg-slate-800 p-5 rounded-2xl border-2 border-blue-500/40 text-2xl font-black shadow-inner focus:border-blue-400 focus:outline-none transition-all">
                                {[1, 5, 10].map(c => <option key={c} value={c}>{c}</option>)}
                        </select>
                    </div>
                </div>
                <QuickBetButtons wager={wager} setWager={setWager} balance={balance} disabled={isDropping} />
                <motion.button 
                    whileHover={{ scale: 1.03, filter: "brightness(1.1)" }}
                    whileTap={{ scale: 0.97 }}
                    transition={{ type: "spring", stiffness: 400, damping: 17 }}
                    onClick={dropCandies} disabled={isDropping || wager * ballCount <= 0 || wager * ballCount > balance} 
                    className="w-full mt-4 bg-gradient-to-r from-blue-500 via-purple-500 to-blue-500 text-white p-6 rounded-2xl text-3xl font-black shadow-2xl disabled:opacity-50 disabled:hover:scale-100 transition-all">
...
// --- Game: Sour Apple (High Volatility, 85% RTP) ---
const SourApple = ({ balance, setBalance, playSound, logGameResult }) => {
    const GRID_SIZE = 5;
    const [wager, setWager] = useState(10);
    ...
    return (
        <div className="flex flex-col items-center p-8 bg-gradient-to-br from-green-900/90 via-slate-900 to-slate-900 rounded-3xl shadow-2xl w-full max-w-2xl mx-auto border-2 border-green-500/30 premium-shadow animate-scale-in">
            <Confetti show={showConfetti} />
            <div className="relative w-full aspect-square bg-gradient-to-br from-green-950 to-emerald-950 rounded-3xl mb-6 grid grid-cols-5 gap-3 p-5 border-4 border-green-500/30 shadow-2xl">
                {gameState === 'busted' && (
                    <div className="absolute inset-0 bg-black/90 z-10 flex flex-col items-center justify-center rounded-3xl backdrop-blur-sm animate-scale-in">
                        <div className="text-8xl font-black text-red-500 mb-4 animate-shake">SOUR!</div>
                        <div className="text-2xl text-red-300 mb-6">Better luck next time!</div>
                ...
                {Array.from({ length: GRID_SIZE*GRID_SIZE }).map((_, i) => (
                    <button key={i} onClick={() => handleTileClick(i)} disabled={gameState !== 'playing' || revealed.has(i)} 
                        className={`w-full h-full rounded-2xl flex items-center justify-center text-5xl transition-all duration-300 shadow-xl ${
                            revealed.has(i) 
            ...
            {isBetting ? (
                 <div className="w-full">
                     <div className="grid grid-cols-2 gap-3 mb-3">
                         <div>
                            <label className="text-sm text-slate-300 mb-2 block font-bold">Bet Amount</label>
                            <input type="number" value={wager} onChange={e => setWager(Math.max(0, Math.min(balance, parseInt(e.target.value) || 0)))} className="w-full bg-slate-800 p-5 rounded-2xl text-2xl font-black border-2 border-green-500/40 shadow-inner focus:border-green-400 focus:outline-none transition-all" />
                         </div>
                         <div>
                            <label className="text-sm text-slate-300 mb-2 block font-bold">Sour Apples</label>
                            <select value={numBadApples} onChange={e => setNumBadApples(Number(e.target.value))} className="w-full bg-slate-800 p-5 rounded-2xl font-black text-2xl border-2 border-green-500/40 focus:border-green-400 focus:outline-none transition-all">
                                {[1, 3, 5, 10, 15, 20, 24].map(n => <option key={n} value={n}>{n} Sour</option>)}
                            </select>
                         </div>
                     </div>
                     <QuickBetButtons wager={wager} setWager={setWager} balance={balance} disabled={false} />
                     <button onClick={startGame} disabled={wager <= 0 || wager > balance} className="w-full mt-4 bg-gradient-to-r from-green-500 via-emerald-500 to-green-500 text-white p-6 rounded-2xl text-3xl font-black shadow-2xl hover:scale-105 hover:shadow-green-500/50 disabled:opacity-50 disabled:hover:scale-100 transition-all">
                         START PICKING
                     </button>
                </div>
            ) : (
                <div className="w-full">
                    <div className="text-center mb-4 p-6 glass-effect rounded-2xl border-2 border-green-400/30">
                        <div className="text-sm text-green-300 mb-1 font-bold">Current Multiplier</div>
                        <div className="text-6xl font-black text-green-400 mb-2 tabular-nums">{currentMultiplier}x</div>
                        <div className="text-lg text-yellow-300 font-bold">Next: {calculateMultiplier(goodApplesFound + 1)}x</div>
        ...
        <AnimatePresence>
            ...
            >
                <motion.div
                    ...
                    className="glass-effect rounded-3xl p-8 border-2 border-purple-500/30 premium-shadow w-full max-w-sm"
                    onClick={(e) => e.stopPropagation()}
                >
                    <h2 className="text-3xl font-black text-center mb-2 text-yellow-300">{isLogin ? 'Welcome Back!' : 'Create Account'}</h2>
                    ...
                    <form onSubmit={handleSubmit}>
                        <div className="mb-4">
                            ...
                            <input 
                                ...
                                className="w-full bg-slate-800 p-3 rounded-xl border-2 border-purple-500/40 shadow-inner focus:border-purple-400 focus:outline-none transition-all"
                        ...
                        <motion.button 
                            ...
                            type="submit"
                            disabled={isLoading}
                            className="w-full bg-gradient-to-r from-yellow-500 to-orange-500 text-slate-900 p-4 rounded-xl text-lg font-black shadow-lg hover:shadow-yellow-500/40 disabled:opacity-50 transition-all"
                        >
                    ...
                    <div className="text-center mt-6">
                        <button onClick={() => { setIsLogin(!isLogin); setError(''); }} className="text-sm text-slate-400 hover:text-yellow-300 transition-colors">
...
// --- Main App Component with Game Selection ---
export default function App() {
    ...
    // Effect to update local storage when user or balance changes
    useEffect(() => {
        if (user) {
            try {
    ...
    const handleAuthSuccess = (userData) => {
        setUser(userData);
        setBalance(userData.balance);
        // Local storage will be updated by the useEffect hook
    };
    ...
    const handleLogout = () => {
        ...
        try {
            ...
            console.error("Could not remove user session from local storage:", error);
        }
    };


    const gameComponents = {
        slots: <FruitFrenzy balance={balance} setBalance={setBalance} playSound={playSound} logGameResult={logGameResult} />,
        scratch: <SugarScratch balance={balance} setBalance={setBalance} playSound={playSound} logGameResult={logGameResult} />,
        rocket: <RocketKebab balance={balance} setBalance={setBalance} playSound={playSound} logGameResult={logGameResult} />,
        candy: <CandyDrop balance={balance} setBalance={setBalance} playSound={playSound} logGameResult={logGameResult} />,
        apple: <SourApple balance={balance} setBalance={setBalance} playSound={playSound} logGameResult={logGameResult} />,
    };

    const navItems = [
        { id: 'slots', label: 'Fruit Frenzy', icon: <SlotIcon />, color: 'from-purple-500 to-purple-600', desc: 'Classic slots with massive jackpots', gradient: 'from-purple-900 to-slate-900' },
        { id: 'scratch', label: 'Sugar Scratch', icon: <ScratchIcon />, color: 'from-pink-500 to-pink-600', desc: 'Instant wins up to $250,000', gradient: 'from-pink-900 to-slate-900' },
        { id: 'rocket', label: 'Rocket Kebab', icon: <RocketIcon />, color: 'from-orange-500 to-orange-600', desc: 'Cash out before the crash', gradient: 'from-orange-900 to-slate-900' },
        { id: 'candy', label: 'Candy Drop', icon: <CandyIcon />, color: 'from-blue-500 to-blue-600', desc: 'Watch candy bounce to riches', gradient: 'from-blue-900 to-slate-900' },
        { id: 'apple', label: 'Sour Apple', icon: <AppleIcon />, color: 'from-green-500 to-green-600', desc: 'Avoid the sour, find the sweet', gradient: 'from-green-900 to-slate-900' },
    ];

    const handleRefill = () => { if (balance <= 0) { playSound('playBigWin'); setBalance(1000); } };

    return (
        <main className="bg-slate-900 text-white min-h-screen p-4 flex flex-col items-center relative overflow-hidden">
            ...
            <div className="fixed inset-0 opacity-20 pointer-events-none">
                ...
                <div className="absolute top-1/2 left-1/2 w-96 h-96 bg-pink-500 rounded-full blur-3xl animate-float" style={{animationDelay: '2s'}}></div>
            </div>

            <header className="w-full max-w-6xl mb-6 glass-effect p-5 rounded-2xl shadow-2xl z-10 animate-slide-up">
                <div className="flex justify-between items-center flex-wrap gap-4">
                    <div className="flex items-center gap-4">
                        <h1 className="text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 via-pink-500 to-purple-500 animate-neon">Gourmet Fun</h1>
                        {!showSelection && <button onClick={backToSelection} className="text-sm bg-slate-700 hover:bg-slate-600 px-4 py-2 rounded-lg font-bold transition-all">← Back</button>}
                    </div>
                    <div className="flex items-center gap-3">
                         {user ? (
                             <>
                                <div className="text-2xl font-black bg-gradient-to-r from-slate-800 to-slate-700 px-6 py-3 rounded-full border-2 border-yellow-500/30 shadow-inner flex items-center gap-2 tabular-nums">
                                     <LoyaltyPointsIcon /> <AnimatedBalance value={balance} />
                                </div>
                                <div className="text-sm font-bold text-slate-300">Welcome, {user.username}!</div>
                                <button onClick={handleLogout} className="text-sm bg-red-500 hover:bg-red-400 px-4 py-2 rounded-lg font-bold transition-all">Logout</button>
                             </>
                         ) : (
                            <motion.button 
                                ...
                                className="bg-gradient-to-r from-yellow-500 to-orange-500 text-slate-900 px-6 py-3 rounded-xl font-black shadow-lg"
                            >
                                Login / Sign Up
                            </motion.button>
                         )}
                        ...
                        <button onClick={toggleMute} className="text-slate-400 hover:text-yellow-400 bg-slate-800/50 hover:bg-slate-700/50 p-3 rounded-full transition-all shadow-lg hover:scale-110">{isMuted ? '🔇' : '🔊'}</button>
                    </div>
                </div>
            </header>
            ...
            {showSelection ? (
                <div className="w-full max-w-6xl z-10 animate-fade-in">
                    <div className="text-center mb-8">
                        <h2 className="text-5xl font-black mb-3 text-transparent bg-clip-text bg-gradient-to-r from-yellow-300 to-pink-400">Choose Your Game</h2>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {navItems.map((item, index) => (
                            <button 
                                ...
                                className="group relative glass-effect p-8 rounded-3xl hover:scale-105 transition-all duration-300 premium-shadow hover:border-2 hover:border-yellow-400/50 animate-scale-in overflow-hidden"
            ...
            {!audioInitialized && <div className="fixed bottom-6 right-6 glass-effect text-white px-6 py-4 rounded-xl shadow-2xl animate-pulse z-50 border border-yellow-400/50">
                <div className="flex items-center gap-3">
